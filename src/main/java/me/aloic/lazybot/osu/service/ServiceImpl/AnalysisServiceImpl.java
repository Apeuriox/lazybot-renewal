...
public class AnalysisServiceImpl implements AnalysisService
{
    ...
    {
        double recommended = Math.pow(params.getPlayerInfo().getStatistics().getPp(), 0.4) * 0.195;
        double recommendedFix=Math.pow((params.getPlayerInfo().getStatistics().getPp() * 0.85 / 20.0),0.33333);
        return "ppy说"+params.getPlayerInfo().getUsername() +"应该打" + CommonTool.toString(recommended) + "星的图\n"
                    +"另一种可能说" + params.getPlayerInfo().getUsername()+ "应该打"+CommonTool.toString(recommendedFix) + "星的图";
