...
public class OsuOAuthService
{
    ...
    private final OAuthLinkSessionService sessionService;
    ...
    {
        PlatformIdentityPO identity = identityService.ensurePlatformIdentity(
        ...
        sessionMapper.invalidateOutstanding(identity.getId(), LocalDateTime.now());
        OAuthLinkSessionPO session = new OAuthLinkSessionPO();
        session.setState_hash(OAuthLinkSessionService.hash(state));
        session.setPlatform_identity_id(identity.getId());
        session.setExpires_at(LocalDateTime.now().plusMinutes(10));
    ...
    public PlayerInfoDTO completeAuthorization(String code, String state)
    {
        ...
        OAuthLinkSessionPO session = sessionService.consume(state);
