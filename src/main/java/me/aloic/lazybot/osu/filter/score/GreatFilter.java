...
@AllArgsConstructor
public class GreatFilter implements ScoreFilter
{
    ...
    public boolean filter(ScoreLazerDTO score) {
        return ScoreFilter.numericComparison(operator, Optional.ofNullable(score.getStatistics().getGreat()).orElse(0), threshold);
    }
