...
@AllArgsConstructor
public class MehFilter implements ScoreFilter
{
    ...
    public boolean filter(ScoreLazerDTO score) {
        return ScoreFilter.numericComparison(operator, Optional.ofNullable(score.getStatistics().getMeh()).orElse(0), threshold);
    }
