...
/*
 * 同时处理两种重复消息（OneBot / Tencent 共用）：
 * 1. 同一条消息被重复推送（messageId）；
...
public class MessageDeduplicator
{
    ...
    public void replicateCheck(LazybotSlashCommandEvent event)
    {
        if (!Boolean.TRUE.equals(event.getIstSlashCommand())) {
            return;
        }
        String deliveryKey = tencentDeliveryKey(event);
        if (deliveryKey != null && !reserveDelivery(deliveryKey)) {
            logger.info("Tencent 消息 {} 已处理，跳过重复上报", deliveryKey);
            return;
        }

        String commandKey = tencentCommandKey(event);
        if (inFlightCommands.putIfAbsent(commandKey, true) != null) {
            completeDelivery(deliveryKey);
            logger.info("{} 指令正在排队或执行，已合并重复请求", event.getCommandType());
            return;
        ...
        logger.info("{} 指令进入用户队列", event.getCommandType());
        try {
            String userKey = "tencent:" + event.getPlatformUserId();
            Optional<CompletableFuture<Void>> submitted = commandSequencer.submit(
                    userKey,
                    () -> slashCommandProcessor.processTencent(event));
            if (submitted.isEmpty()) {
                inFlightCommands.remove(commandKey, true);
                removeDelivery(deliveryKey);
    ...
    private static String tencentDeliveryKey(LazybotSlashCommandEvent event)
    {
        if (event.getSourceMessageId() == null || event.getSourceMessageId().isBlank()) {
            return null;
        }
        String channel = event.getPlatformChannelId() == null ? "c2c" : event.getPlatformChannelId();
        return "tencent:" + channel + ":" + event.getSourceMessageId();
    }

    private static String tencentCommandKey(LazybotSlashCommandEvent event)
    {
        String channel = event.getPlatformChannelId() == null ? "c2c" : event.getPlatformChannelId();
        return "tencent:" + event.getPlatformUserId() + ":" + channel + ":" + event.getCommandString();
    }
}
